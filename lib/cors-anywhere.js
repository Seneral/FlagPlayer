...
function withCORS(headers, request) {
  if (request.corsAnywhereRequestState.copyOrigin)
  	headers['access-control-allow-origin'] = request.corsAnywhereRequestState.origin || request.corsAnywhereRequestState.allowOrigin;
  else 
    headers['access-control-allow-origin'] = request.corsAnywhereRequestState.allowOrigin;
  
  if (request.corsAnywhereRequestState.allowCredentials)
  	headers['access-control-allow-credentials'] = true;

  if (request.corsAnywhereRequestState.passCookies)
    headers['x-set-cookies'] = JSON.stringify(headers['set-cookie'] || headers['set-cookie2'] || []);
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    req.corsAnywhereRequestState = {
      ...
      allowOrigin: corsAnywhere.allowOrigin,
      copyOrigin: corsAnywhere.copyOrigin,
      passCookies: corsAnywhere.passCookies,
    ...
    if (corsAnywhere.setMode && req.headers["x-mode"]) {
      if (req.headers["x-mode"] == "navigate") {
        ...
        req.headers["origin"] = location.protocol + "//" + location.host;
      }
      delete req.headers["x-mode"];
    }

    if (corsAnywhere.passCookies && req.headers["x-cookies"]) {
      req.headers["cookie"] = req.headers["x-cookies"];
      delete req.headers["x-cookies"];
    }
